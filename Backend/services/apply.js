...
module.exports = {
    ...
    find(value) {
        return new Promise((resolve, reject) => {
            ...
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table.a}`, 
                select: `SELECT * FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id`
            };
    ...
    findowner(value ,tb_owner_o_id) {
        return new Promise((resolve, reject) => {
            const limitPage = config.limitPage;
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id = ${connection.escape(tb_owner_o_id)}`, 
                select: `SELECT * FROM tb_apply JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id = ${connection.escape(tb_owner_o_id)}`
            };
