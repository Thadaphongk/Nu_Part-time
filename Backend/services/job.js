...
module.exports = {
    ...
    find(value) {
        return new Promise((resolve, reject) => {
            ...
            const sqls = {
                count: 'SELECT COUNT(*) AS row_count FROM tb_detailjob',
           ...
           // เรียงลำดับข้อมูล
            sqls.select += ' ORDER BY j_created DESC';
    ...
    findHistory(value,tb_owner_o_id) {
        return new Promise((resolve, reject) => {
            const limitPage = config.limitPage;
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table} WHERE tb_owner_o_id = ${connection.escape(tb_owner_o_id)}`,
                select: `SELECT * FROM ${table} WHERE tb_owner_o_id = ${connection.escape(tb_owner_o_id)}`
            };
