...
router.get('/manage',isInRolesowner(['owner']), [
    query('page').isInt()
], async (req, res) => {
    try {
        req.validate();
        res.json(await applyService.findowner(
            req.query,
...
router.put('/manage/:id',isInRolesowner(['owner']), [
    param('id').isInt(),
...
// ลบข้อมูลห้องประชุม
router.delete('/manage/:id',isInRolesowner(['owner']), [
    param('id').isInt()
], async (req, res) => {
    try {
        req.validate();
